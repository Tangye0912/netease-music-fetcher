...
- **官网扫码登录（浏览器）成为唯一登录方式**：新增 `music_fetch/browser_login.py`，启动本机 Chrome/Edge（临时 profile）打开网易云官网登录页，扫码官网二维码（真实浏览器生成，可绕开工具二维码被风控标记的问题）后通过 DevTools 协议自动取回登录凭证。默认**直接打开扫码窗口**（一次扫码即完成）；`run_official_login(reuse_existing=True)` 可尝试复用真实浏览器已保存的登录态（在部分 Edge 上不可用）。**移除终端二维码登录与粘贴 Cookie 两个入口**；新增 `websocket-client` 依赖。
