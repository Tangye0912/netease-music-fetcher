...
- **任务队列跨重启保留**：未完成任务持久化到 `~/.config/music-fetch/queue.json`（原子写、私有权限），重启后自动恢复；恢复时目标文件已存在的任务直接记为完成，不再重复下载（也是后续"增量下载"的地基）。退出时不再把未开始的任务记为"已取消"——它们会留到下次运行；正在下载的任务仍取消并如实记录。
- **任务页实时化**：表格每 0.5 秒自动刷新（运行中任务显示 已下载/总量/百分比），不再依赖手动回车；任务详情对运行中任务实时显示阶段（解析地址/下载/转码/写标签/歌词）、进度与速度。
- **批次分组视图**：任务页新增批次列（B1/B2…，单曲标注"单曲"），`g` 键切换按批次分组视图，小节头显示各批次完成度。
- **下载结果通知**：回到主菜单时汇总播报新增的 成功/失败/取消 数量；登录状态改为后台校验，首屏不再等待网络，校验结果（欢迎回来/已过期/网络异常）通过通知显示。

### Changed

- **Ctrl+C 语义修正**：子页面的 Ctrl+C 现在真的返回主菜单（此前页脚提示"返回"但实际会退出）；主菜单提示改为"Ctrl+C 退出程序"。
- **任务页按键统一**：翻页改为与其他页面一致的 `n/p`，暂停全部/恢复全部改为大写 `P`/`R`（提示文案同步更新）。
- **GBK 控制台回退加固**：无 ANSI 能力时的纯文本输出会替换无法编码的字符（✓/✕ 此前会导致二次崩溃）。

### QA

- 回归测试：`python3 -m pytest tests/ -q`（501 通过，10 个参数化子测试通过）。
- 覆盖率：通过 75% 门槛。
- 静态检查：`mypy --strict` 零错误、ruff 全部通过。
- Windows/SSH 真机冒烟仍未执行（见 ROADMAP）。

## v3.5.0 (2026-09-12)

### Added

- **菜单后台任务队列**：单曲、搜索、歌单/专辑批量和历史重试提交后立即返回；任务页支持分页、暂停、恢复、取消、重试、打开目录与批次 CSV 导出。输入底栏自动刷新，各下载共用并发上限。
- **队列恢复与退出**：登录失效保留任务等待重新登录；新任务预留文件名避免并发冲突；退出时取消待办并等待工作线程清理完成。会话与历史文件字段保持兼容，任务队列仅在本次运行中保留。
...
- **无损与 Hi-Res**：可播放候选新增 `lossless`/`hires` FLAC 档位；VIP 账号可获得对应地址，不可用时沿用现有候选链自动降级。
- **专辑下载**：支持网易云专辑链接与分享文案，TUI、CLI 和批量识别均可展开专辑曲目并下载。
- **双语歌词**：单曲支持原文、翻译、双语三种歌词模式；双语 LRC 按时间戳合并，CLI 新增 `--lyric-mode`。
- **明暗主题**：TUI 设置新增深色/浅色主题，菜单、表格、信息卡片、选择弹窗与进度条统一切换并持久化。

### Changed

- **移除脚本模式（CLI）**：`music-fetch` 现仅有 TUI 交互界面一种入口，传参数时提示"脚本模式已移除"并退出；删除 `cli.py`、CLI 专属测试与相关公开 API（`run_download`/`load_cookie`/`DownloadResult` 等），开发重心转向 TUI 体验。
- **批量下载闭环**：下载结束显示成功、失败、取消、输出目录和失败原因 Top 3 汇总面板，并提示从下载历史重试。
- **我的歌单分页**：API 自动聚合最多 500 个歌单；界面每页显示 10 个，支持 `n`/`p` 翻页。
- **工程质量**：CI 新增 75% 覆盖率门槛、严格 mypy 与 ruff；清理无生产引用的旧 API、音频和任务辅助代码。

### Fixed

- 修复搜索与歌词 API 返回 `null` 对象时触发 `AttributeError` 的问题。
- 修复音频或歌词标签写入失败会把已完成下载误判为失败的问题；元数据失败现在只记录警告。
- 修复 Windows 控制字符和 `CON`/`PRN`/`AUX`/`NUL`/`COM1-9`/`LPT1-9` 等保留文件名。
- 修复单条未知异常会中断整个批量识别的问题。
- 断点续传新增 `.part.src` 来源旁车校验，媒体 URL 变化时不再拼接旧来源字节。

### QA

- 回归测试：`python3 -m pytest tests/ -q`（403 通过，2 跳过，10 个参数化子测试通过）。
- 覆盖率：总覆盖率 76.72%，通过 75% CI 门槛。
- 静态检查：`mypy --strict`（56 个源文件零错误）与 ruff 全部通过。
- 三平台 PyInstaller 构建纳入 CI 发布验收。
...
- **检查更新支持鉴权**：支持 `MUSIC_FETCH_GITHUB_TOKEN` / `GITHUB_TOKEN` 环境变量；报错区分"匿名限流（403）""私有仓库（404）""网络不可用"三类，提示更准确。

### Changed

- **终端窗口体验**：启动脚本预设终端大小（Windows 140×40 / macOS xterm 序列，用户可随时拉伸）；内容宽度上限从 100 提到 160 列，拉宽窗口能显示更多；表格被截断时底部显示加宽提示。
- **搜索分页**：搜索结果每页 10 条（`n`/`p` 翻页、`0` 返回），一屏放得下，不再滚动丢失内容。
- **批量结果排序**：重复行现在追加到结果末尾，不再先于原曲出现。
- **TUI 视觉重构**：居中全宽标题、青色状态带、带边框菜单/表格、黄绿红灰信息层级、图标化状态提示，以及统一的多选框和进度条深色主题。
...
- 修复检查更新把 GitHub 匿名频率限制（403）误报为"私有仓库"的问题。
- 移除误提交的 `.reasonix` 工具元数据文件，并在 `.gitignore` 中忽略该目录。
- README 当前版本、登录能力与章节编号同步到 3.2.0。

### QA

- 回归测试：`python3 -m pytest tests/ -q`（393 通过，2 跳过，15 个参数化子测试通过）。
