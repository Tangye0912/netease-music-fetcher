...
### 1.1 v0.7.0 相对 v0.6.0 的更新

- **修复**：批量下载中 worker 异常结束时遗漏写入下载历史记录。
- **修复**：批量检测完成后不再无条件重置歌曲选中状态，保持默认全选可下载项。
- **修复**：CLI 移除无效的 `--format` 参数（之前被接受但无法使用）。
- **改进**：下载取消标志改用 `threading.Event`，确保跨线程可见性。
- **改进**：清理 `main.py` 中约 15 处重复局部导入，移除未使用的模块导入。
- **改进**：`fetch_song_metadata` 网络错误时添加日志记录，不再静默吞错。
- **测试**：修复脆弱的字符串匹配断言，Windows 平台跳过 shell wrapper 测试。
...
| `tests/` | 86 个单元/回归测试，覆盖 API、解析、存储、批量结果、入口脚本和下载 worker。 |
