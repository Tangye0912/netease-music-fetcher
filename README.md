...
- **任务队列跨重启保留**：未完成任务持久化并在下次启动时恢复；目标文件已存在的任务直接记为完成。
- **任务页实时化**：表格每 0.5 秒自动刷新；任务详情实时显示阶段（解析/下载/转码/写标签/歌词）、进度与速度。
- **批次分组视图**：任务页显示批次列，`g` 键切换按批次分组。
- **通知与交互**：回到主菜单播报下载结果；登录校验后台化不再阻塞首屏；子页面 Ctrl+C 返回主菜单；任务页翻页键统一为 n/p（暂停/恢复全部为 P/R）。

### 1.2 v3.5.0 变更

- **后台任务队列**：单曲、搜索、歌单/专辑批量和历史重试提交后立即返回，可边下载边继续搜索；主菜单新增「下载任务」页，支持分页、暂停/恢复、取消、重试、打开目录、登录恢复与批次 CSV 导出；输入底栏实时显示下载进度。
- **下载历史增强**：「重试全部失败」将筛选范围内的失败记录提交后台重试；「清空历史」仅清空记录不删文件；每页 15 条、序号直选，不再超屏。
- **其他**：批量识别结果下载前可试听任意可下载歌曲；移除被任务队列取代的旧批量会话模块（`BatchDownloadSession`）。
